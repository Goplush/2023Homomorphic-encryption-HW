    ...
    def onShow(self):
        for i in range(self.rownum):
            if self.radioButtons[i].isChecked() == True:
                ...
                res = showVoteResult(captcha, total, prikey, 1)
                drawResult(res)
    ...
    def onShow(self):
        for i in range(self.rownum):
            if self.radioButtons[i].isChecked() == True:
                ...
                res = showVoteResult(captcha, total, prikey, 0)
                drawResult(res)
...
if __name__ == "__main__":
    ...
    el_viewWindow = EL_ViewWindow(usr)
    el_viewWindow.show()
    pa_viewWindow = PA_ViewWindow(usr)
    pa_viewWindow.show()
    sys.exit(app.exec_())
